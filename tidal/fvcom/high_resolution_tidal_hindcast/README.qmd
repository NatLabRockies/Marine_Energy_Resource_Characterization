---
title: H20 High Resolution Tidal Hindcast
...
# This is a hack to get around how quarto handles inline code blocks and backticks
def render_monospace(text):
...
This repository contains the code and methodology for processing and visualizing the U.S. Department
of Energy (DOE) Hydropower and Hydrokinetic Office (H20) high-resolution tidal hindcast data generated
using the Finite Volume Community Ocean Model (FVCOM) at five strategically selected U.S. coastal
locations. The project represents a collaborative effort between the [Pacific Northwest National
Laboratory][pnnl_resource_char] (PNNL) (data generation) and the [National Laboratory of the Rockies][nlr_resource_char] (data processing and visualization), to generate, standardize, and
summarize original high resolution tidal data into accessible resource data for tidal resource
characterization. Complete standardized and summarized datasets can downloaded from the
[AWS S3 Open Energy Data Initiative Marine Energy Data Lake][aws_tidal_data], 
and summary data is visualized on the [Marine Energy Atlas][me_atlas].

# Versions

| Asset                             | Status    | Version                       |
| ---                               | ---       | ---                           |
| Processing Code (This Repository) | In Review | `{python} code['version']`    |
| Public Dataset                    | Released  | `{python} dataset['version']` |
...
| Project ID                        | `{python} render_monospace(dataset['name'])` |
...
```{python}
#| echo: false
#| label: tbl-loc-overview
#| tbl-cap: Location Overview
...
for key, this in loc_spec.items():
    ...
    face_count.append(this["face_count"])
...
df = pd.DataFrame(
    {
        ...
        "Start Time [UTC]": start_date,
        "End Time [UTC]": end_date,
...
| ------------                   | -------------                                                                                                                                    |
| Model                          | [Finite Volume Community Ocean Model (FVCOM) `{python} model['model_version'].replace("FVCOM_", "")`](htpps://www.fvcom.org)                     |
...
: High Resolution Tidal Hindcast FVCOM Specification and Configuration {#tbl-model-config}
...
: High Resolution Tidal Hindcast IEC Standards Compliance {#tbl-iec-compliance}
...
| Surface Elevation                     | m [from [NAVD88][navd88]]  | face, time              | Distance from surface elevation to NAVD88 elevation |
...
: High Resolution Tidal Hindcast Variables {#tbl-model-config}
...
![Cook Inlet, Alaska - Sea Water Speed, Yearly Average, Depth Average](./docs/img/AK_cook_inlet_mean_sea_water_speed.png){#fig-ak-cook-speed}

![Aleutian Islands - Sea Water Speed, Yearly Average, Depth Average](./docs/img/AK_aleutian_islands_mean_sea_water_speed.png){#fig-ak-aleutian-speed}

![Western Passage, Maine - Sea Water Speed, Yearly Average, Depth Average](./docs/img/ME_western_passage_mean_sea_water_speed.png){#fig-me-west-speed-speed}

![Piscataqua River New Hampshire - Sea Water Speed, Yearly Average, Depth Average](./docs/img/NH_piscataqua_river_mean_sea_water_speed.png){#fig-nh-piscataqua-speed}

![Puget Sound, Washington - Sea Water Speed, Yearly Average, Depth Average](./docs/img/WA_puget_sound_mean_sea_water_speed.png){#fig-wa-puget-speed}
...
{{< include "me_atlas_high_resolution_tidal_data_qoi_visualization_specification.md" >}}
...
[aws_tidal_data]: https://data.openei.org/s3_viewer?bucket=marine-energy-data
[me_atlas]: https://maps.nrel.gov/marine-energy-atlas/data-viewer/data-library/layers?vL=WavePowerMerged
[wpto_resource_char]: https://www.energy.gov/eere/water/marine-energy-resource-assessment-and-characterization
[pnnl_resource_char]: https://www.pnnl.gov/marine-energy-resource-characterization
[nlr_resource_char]: https://www.nrel.gov/water/resource-characterization
[navd88]: https://en.wikipedia.org/wiki/North_American_Vertical_Datum_of_1988
[this_repo]: https://github.com/NLR/Marine_Energy_Resource_Characterization
